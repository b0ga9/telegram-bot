...
async def publish_command(
    ...
    """Open the publication editor using the latest prepared post."""
    if not await admin_only(update):
        return
    ...
    text = (
        ...
        or context.user_data.get("publish_text")
        or ""
    )

    if not text:
        await update.message.reply_text(
            "📝 Сначала создай /news или /pulse."
        )
        return
    ...
    # Если к посту уже привязано изображение, не спрашиваем заново
    # «текст или фото»: сразу открываем редактор.
    if (
        context.user_data.get("publish_photo")
        or context.user_data.get("publish_photo_path")
    ):
