...
def _header(d, section, title):
    # Компактная фирменная шапка: TRD слева, раздел справа.
    d.text((72, 52), "TRD", font=_font(42, True), fill=WHITE)
    d.rounded_rectangle((205, 58, 430, 105), radius=14, fill=PANEL)
    d.text((228, 68), section, font=_font(20, True), fill=MUTED)
    d.text((72, 155), title, font=_font(58, True), fill=TEXT)


def _footer(d):
    now = datetime.now(timezone.utc).strftime("%H:%M UTC")
    d.text((72, 970), "TRD • ДАННЫЕ РЫНКА", font=_font(17, True), fill=MUTED)
    d.text((1325, 970), now, font=_font(17), fill=MUTED)
...
def build_pulse_card(market: dict[str, Any], signal: Optional[dict[str, Any]] = None,
                     output_dir="/tmp/trd_visuals"):
    """Понятная минималистичная карточка PULSE без лишней графики."""
    img = Image.new("RGB", (W, H), BG)
    d = ImageDraw.Draw(img)
    ...
    labels = {
        "BROAD_RALLY": ("🟢 ШИРОКИЙ РОСТ", GREEN, "Большинство рынка движется вверх"),
        "BROAD_SELLOFF": ("🔴 ШИРОКОЕ СНИЖЕНИЕ", RED, "Большинство рынка движется вниз"),
        "BTC_LED_MOVE": ("🟡 ДВИЖЕНИЕ ВОКРУГ BTC", YELLOW, "Bitcoin задаёт направление рынку"),
        "ALTCOIN_ROTATION": ("🟢 АКТИВНОСТЬ АЛЬТКОИНОВ", GREEN, "Капитал перемещается между альткоинами"),
        "MARKET_DIVERGENCE": ("🟡 РАСХОЖДЕНИЕ", YELLOW, "Крупные монеты и рынок движутся неодинаково"),
        "NEUTRAL": ("⚪ СТАБИЛИЗАЦИЯ", MUTED, "Ярко выраженного движения пока нет"),
    }
    state, accent, explanation = labels.get(
        regime, ("⚪ ДВИЖЕНИЕ РЫНКА", MUTED, "Рынок меняется")
    )

    _box(d, (72, 270, 1464, 410), radius=22)
    d.text((108, 300), state, font=_font(34, True), fill=accent)
    d.text((108, 354), explanation, font=_font(21), fill=MUTED)

    btc = market.get("btc") or {}
    eth = market.get("eth") or {}
    btc_ch = signal.get("btc_metrics", {}).get("1h")
    eth_ch = signal.get("eth_metrics", {}).get("1h")

    # Two clean rows: price + 1h + 24h, so the picture explains the move.
    rows = [
        ("BTC", btc, btc_ch),
        ("ETH", eth, eth_ch),
    ]
    y = 445
    for name, coin, ch1h in rows:
        _box(d, (72, y, 930, y + 125), radius=20, fill=PANEL2)
        d.text((108, y + 25), name, font=_font(25, True), fill=MUTED)
        d.text((220, y + 19), _price(coin.get("current_price")), font=_font(34, True), fill=TEXT)

        ch24 = _change(coin, "24h")
        d.text((650, y + 23), "1 ЧАС", font=_font(16, True), fill=MUTED)
        d.text((720, y + 18), _pct(ch1h), font=_font(24, True),
               fill=GREEN if _n(ch1h) > 0 else RED if _n(ch1h) < 0 else MUTED)
        d.text((650, y + 68), "24 ЧАСА", font=_font(16, True), fill=MUTED)
        d.text((750, y + 63), _pct(ch24), font=_font(22, True),
               fill=GREEN if _n(ch24) > 0 else RED if _n(ch24) < 0 else MUTED)
        y += 145

    breadth = signal.get("breadth", {})
    _box(d, (970, 445, 1464, 695), radius=20)
    d.text((1010, 478), "ШИРИНА РЫНКА", font=_font(20, True), fill=MUTED)
    pos = _n(breadth.get("positive_pct"))
    neg = _n(breadth.get("negative_pct"))
    d.text((1010, 530), f"🟢 РАСТУТ  {pos:.0f}%", font=_font(27, True), fill=GREEN)
    d.text((1010, 590), f"🔴 СНИЖАЮТСЯ  {neg:.0f}%", font=_font(27, True), fill=RED)
    d.text((1010, 646), "Шире движение → сильнее сигнал", font=_font(17), fill=MUTED)
