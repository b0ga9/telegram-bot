...
async def publish_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update):
        return
    text = context.user_data.get("pulse_text") or context.user_data.get("news_text") or context.user_data.get("publish_text") or ""
    if not text:
        await update.message.reply_text("📝 Сначала создай /news или /pulse.")
        return
...
async def market_command(
    ...
    try:
        ...
        btc = data.get("btc") or {}
        eth = data.get("eth") or {}
        cap_change = data.get("market_cap_change_24h")

        if (cap_change or 0) <= -1:
            mood = "🔴"
            title = "Рынок остаётся под давлением"
            opinion = "Пока продавцы контролируют движение, устойчивого разворота не видно."
        elif (cap_change or 0) >= 1:
            mood = "🟢"
            title = "Рынок сохраняет положительный импульс"
            opinion = "Широкий рост выглядит конструктивно, если спрос продолжит удерживаться."
        else:
            mood = "🟡"
            title = "Рынок движется без выраженного импульса"
            opinion = "Сейчас важнее дождаться подтверждения направления по BTC и общей капитализации."

        market_text = (
            f"{mood} <b>TRD MARKET</b>\n\n"
            f"<b>{title}</b>\n\n"
            f"<blockquote>Капитализация рынка — <b>{format_money(data.get('market_cap'))}</b>, "
            f"изменение за 24 часа — <b>{format_pct(cap_change)}</b>. "
            f"BTC — <b>{format_price(btc.get('current_price'))}</b> "
            f"({format_pct(price_change_24h(btc))}), ETH — <b>{format_price(eth.get('current_price'))}</b> "
            f"({format_pct(price_change_24h(eth))}).</blockquote>\n\n"
            f"<b>Мнение TRD:</b> {opinion}"
        )
